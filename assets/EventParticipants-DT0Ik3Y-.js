import{j as e,o as b,k as x,g as i,X as q,P,W as A}from"./index-Baor4_c_.js";import{C as F}from"./check-DRk803d2.js";import{I as R}from"./input-DjR9QGXa.js";import{n as G}from"./index-WQ0BBVcu.js";import{F as p}from"./FormInput-C4lJuSly.js";import{S as J}from"./search-B8jRLNl8.js";function X({checked:a,onChange:u}){return e.jsx("div",{onClick:u,className:"w-8 h-8 border-2 bg-foreground/25 border-primary rounded cursor-pointer",children:e.jsx(b,{children:a&&e.jsx(x.div,{initial:{opacity:0},animate:{opacity:1},exit:{opacity:0},transition:{duration:.1},children:e.jsx(F,{className:"w-full h-full bg-primary text-black"})})})})}function H({isAddOpen:a,eventId:u,onClose:j,onParticipantAdded:y}){const[l,d]=i.useState({name:"",surname:"",email:"",pokemonId:"",nickname:""});i.useEffect(()=>(document.body.style.overflow=a?"hidden":"auto",()=>{document.body.style.overflow="auto"}),[a]);function c(){d({name:"",surname:"",email:"",pokemonId:"",nickname:""}),j()}async function w(s){if(s.preventDefault(),u===null)return;const o={eventId:u,userId:null,name:l.name,surname:l.surname,email:l.email,pokemonId:l.pokemonId||null,nickname:l.nickname||null};try{const m=await fetch("http://localhost:3000/eventParticipants",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)}),h=await m.json();if(!m.ok){console.error(h.message);return}y(),c()}catch(m){console.error(m)}}return e.jsx(b,{children:a&&e.jsx(x.div,{initial:{opacity:0},animate:{opacity:1},exit:{opacity:0},onMouseDown:c,className:`\r
            fixed\r
            inset-0\r
            z-50\r
            items-center\r
